...
for i in range(0, pNpeople):
    for j in range(0, pNhealthp):
        for t in range(0, pNperiods):
            #vBeta[i, j, t] = model.addVar(name='beta_%s_%s_%s' % (i, j, t), vtype=GRB.BINARY)
            vBeta[i, j, t] = model.addVar(ub=pAvailability[i,t], name='beta_%s_%s_%s' % (i, j, t), vtype=GRB.BINARY)

# Binary for person i taking an outward flight in time period t
vAlphaout = {}
for i in range(0, pNpeople):
    for t in range(0, pNperiods):
        vAlphaout[i,t] = model.addVar(name='alphaout_%s_%s' % (i, t), vtype=GRB.BINARY)

# Binary for person i taking a return flight in time period t
vAlpharet = {}
for i in range(0, pNpeople):
    for t in range(0, pNperiods):
        vAlpharet[i,t] = model.addVar(name='alpharet_%s_%s' % (i, t), vtype=GRB.BINARY)

# Binary auxiliary for applying or not the discount in outward flights
vDeltaout = {}
for t in range(0, pNperiods):
    vDeltaout[t] = model.addVar(name='deltaout_%s' % (t), vtype=GRB.BINARY)

# Binary auxiliary for applying or not the discount in return flights
vDeltaret = {}
for t in range(0, pNperiods):
    vDeltaret[t] = model.addVar(name='deltaret_%s' % (t), vtype=GRB.BINARY)

# Number of people taking an outward flight
vNout = {}
for t in range(0, pNperiods):
    vNout[t] = model.addVar(name='nout_%s' % (t), vtype=GRB.INTEGER)

# Number of people taking a return flight
vNret = {}
for t in range(0, pNperiods):
    vNret[t] = model.addVar(name='nret_%s' % (t), vtype=GRB.INTEGER)

# Number of people taking an outward flight with standard fare in time period t
vXstand = {}
for t in range(0, pNperiods):
    vXstand[t] = model.addVar(name='xstand_%s' % (t), vtype=GRB.INTEGER)

# Number of people taking an outward flight with discounted fare in time period t
vXdisc = {}
for t in range(0, pNperiods):
    vXdisc[t] = model.addVar(name='xdisc_%s' % (t), vtype=GRB.INTEGER)

# Number of people taking a return flight with standard fare in time period t
vYstand = {}
for t in range(0, pNperiods):
    vYstand[t] = model.addVar(name='ystand_%s' % (t), vtype=GRB.INTEGER)

# Number of people taking a return flight with discounted fare in time period t
vYdisc = {}
for t in range(0, pNperiods):
    vYdisc[t] = model.addVar(name='ydisc_%s' % (t), vtype=GRB.INTEGER)

# OBJECTIVE FUNCTION
# Objective function definition
model.setObjective(
    quicksum(pCostOut[t] * vXstand[t] for t in range(0,pNperiods-1)) + quicksum((pCostOut[t] - pDiscount)* vXdisc[t] for t in range(0,pNperiods-1)) + quicksum(pCostRet[t] * vYstand[t] for t in range(1,pNperiods)) + quicksum((pCostRet[t] - pDiscount) * vYdisc[t] for t in range(1, pNperiods)),
    sense=GRB.MINIMIZE)
...
for i in range(0, pNpeople):
    model.addConstr(quicksum(t*vAlpharet[i,t] for t in range(1, pNperiods)) - quicksum(t*vAlphaout[i,t] for t in range(0,pNperiods-1)) <= pMaxperiods, 'Maxperiods_%s' %i)
...
# For each pNheathp create a color and assign it to the health profile. Do it with a dictionary
